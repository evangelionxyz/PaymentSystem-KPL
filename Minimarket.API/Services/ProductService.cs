...
{
    private readonly IMongoCollection<Product> _productCollection = client.GetDatabase(settings.Value.DatabaseName).GetCollection<Product>(settings.Value.ProductCollectionName);
    ...
    public async Task<Product?> GetByNameAsync(string name) => await _productCollection.Find(x => x.Name.ToLower() == name.ToLower()).FirstOrDefaultAsync();
