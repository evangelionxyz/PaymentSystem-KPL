...
{
    private readonly IMongoCollection<AuditLog> _logCollection =
        client.GetDatabase(settings.Value.DatabaseName)
              .GetCollection<AuditLog>(settings.Value.AuditLogCollectionName);
    ...
    public async Task<List<AuditLog>> GetAllAsync() =>
        await _logCollection.Find(_ => true).SortBy(x => x.Timestamp).ToListAsync();
}
